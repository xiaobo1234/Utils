...
//						+ bucketName + "---");
                ...
                if(name != null) {
                    if(name.length() > 4) {
                        suffixString = name.substring(name.length()-4,name.length());
                    }else {
                        suffixString = "";
                    }
                }
                ...
                if(!size.equals("0")) {
                    if(suffixString.toLowerCase().equals(".jpg") || suffixString.toLowerCase().equals(".png")) {
                       if(FileUtils.fileIsExists(path) && (new File(path)).isFile()) {
                           boolean isRight = true;
                           String[] str = path.split("/");
                            if(str != null) {//此处判断忽略隐藏文件，android中文件以“.”号开头为隐藏文件
                                for(int i = 0; i < str.length; i++) {
                                      if(str[i].length() > 0 && str[i].substring(0,1).equals(".")) {
                                          isRight = false;
                                      }
                                }
                            }else {
